...
## Archive layout (read this before extracting)

The release archive is **flat**: `package.json`, `src/`, `dist/` and the rest
sit at the archive root, with no wrapping directory. That is what §3 and §7
above assume — they extract straight into the release directory and then run
`npm ci` in it.

```bash
sudo -u webdeploy mkdir -p /d01/fundos-web/releases/2026-08-11
sudo -u webdeploy tar -xzf /tmp/fundos-frontend.tar.gz \
     -C /d01/fundos-web/releases/2026-08-11        # flat: no --strip-components
cd /d01/fundos-web/releases/2026-08-11
ls package.json                                     # must exist HERE
sudo -u webdeploy npm ci --no-fund --no-audit || sudo -u webdeploy npm install --no-fund --no-audit
sudo -u webdeploy npm run build
sudo ln -sfn /d01/fundos-web/releases/2026-08-11/dist /d01/fundos-web/current
```

If `package.json` is not at the release root after extracting, the build never
runs, `releases/<date>/dist` is never created, and the `current` symlink points
at nothing — nginx then serves no index.html and **every page renders blank**,
including the landing page. Check for a stray wrapping directory first:

```bash
tar -tzf /tmp/fundos-frontend.tar.gz | head -3     # expect ./package.json, ./src/, …
ls -l /d01/fundos-web/current/index.html           # must resolve, not dangle
```

A prebuilt `dist/` ships inside the archive as a safety net, so the symlink
target exists even if the build step is skipped. Rebuild anyway when you can —
the shipped `dist/` is only as current as the archive.
