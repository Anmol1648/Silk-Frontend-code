...
function trailForPointer(report: InvestorMatchReport, pointerId: string | null) {
  if (!pointerId) return 'Investor match'
  ...
  switch (pointer.kind) {
    ...
    case 'type': {
      const type =
        report.types.find(item => item.id === pointer.typeId) ??
        report.skips.find(item => item.id === pointer.typeId)
      return type ? type.name : 'Investor type'
    }
    case 'fill': {
      const fill = report.fills.find(item => item.typeId === pointer.typeId)
      return fill ? `How it fills · ${fill.label}` : 'How it fills'
    }
    default:
...
function replyToInvestorAsk(args: {
  text: string
  report: InvestorMatchReport
  pointerId: string | null
}): SilkAiReply {
  const beats = beatsForInvestorPointer(args.report, args.pointerId ?? 'profile')
  return {
    intent: 'explain',
    thinking: ['Reading the book…', 'Checking the mix…'],
    content: [
      beats.why,
      beats.wrong,
      beats.next,
      ...
      'Ask intents for investor what-ifs land next. For now this stays on the written reading.',
    ]
      .filter(Boolean)
      .join('\n\n'),
    proposals: [],
    followUps: investorSuggestions(args.report, args.pointerId).slice(0, 2),
  }
}
...
function Reading({
  ...
  const beats = beatsForInvestorPointer(report, pointerId ?? 'profile')
  return (
    <div className="px-1">
...
export function InvestorsAside({
  report,
  pointerId,
  leaving,
  onClose,
  ...
  const send = (raw?: string) => {
    ...
    setDraft('')
    const reply = replyToInvestorAsk({ text, report, pointerId })
  ...
  const chips = investorSuggestions(report, pointerId)
